...
public class RefreshData extends AsyncTask<JSONArray, Integer, Integer> {
	...
	public RefreshData(Uri uri, ContentResolver cr, String[] mainColumns,
			Long requestid, MyService service, Boolean complex) {
		this.uri = uri;
		this.cr = cr;
	...
	protected Integer doInBackground(JSONArray... params) {
		...
		try{
		if(complex){
			c = cr.query(uri, null, mainColumns[0]+"="+Integer.toString(results.getJSONObject(0).getInt(mainColumns[0])), null, sortOrder);
		}else{
		...
		try {
			...
			if (c.moveToFirst()) {
				for (i = 0; i < length; i++) {
					...
					int k=0;
					while((!c.isAfterLast())&&(!shouldInsert)&&(k<mainColumns.length)&&(c.getInt(c.getColumnIndex(mainColumns[k]))<=obj.getInt(mainColumns[k])))
						k++;
					...
					if ((c.isAfterLast()) || (k<mainColumns.length)) {
						...
						if (j < notPrimaryColumns.size()) {
							...
							for(int l=1;l<mainColumns.length;l++)
								selection=selection.concat(" AND "+mainColumns[l]+"="+c.getString(c.getColumnIndex(mainColumns[l])));
							//currentOperation = currentOperation.withSelection(mainColumn + "=" + c.getString(0), null);
							currentOperation=currentOperation.withSelection(selection, null);

							for (j = 0; j < notPrimaryColumns.size(); j++)
								currentOperation = currentOperation.withValue(
										columns[notPrimaryColumns.get(j)], obj.get(columns[notPrimaryColumns.get(j)]));
				...
				for (i = 0; i < delete.size(); i++) {
					builder.path(uri.getPathSegments().get(0));
					//operations.add( ContentProviderOperation.newDelete( builder.appendPath(delete.get(i).toString()) .build() ).build());
					String selection=new String();
