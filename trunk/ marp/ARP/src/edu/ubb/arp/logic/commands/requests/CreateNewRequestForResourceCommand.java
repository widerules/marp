...
public class CreateNewRequestForResourceCommand extends BaseCommandOperations implements Command {
	private static final Logger logger = Logger.getLogger(CreateNewRequestForResourceCommand.class);
	private JSONArray request = null;
	private JSONArray response = null;
	private DaoFactory instance = null;
	private RequestsDao requestDao = null;
	
	public CreateNewRequestForResourceCommand (JSONArray request) {
	...
	public JSONArray execute() {
		...
		if (!errorCheck(response)) {
			try {
				int requestCreated = requestDao.createNewRequestForResource(week, ratio, senderUserName, targetResourceName, projectName);
				response = addInt("requestcreated", requestCreated, response);
