...
public class UpdateUserRatioCommand extends BaseCommandOperations implements Command {
	private static final Logger logger = Logger.getLogger(UpdateUserRatioCommand.class);
	private JSONArray request = null;
	private JSONArray response = null;
	private DaoFactory instance = null;
	private ProjectsDao projectDao = null;
	
	public UpdateUserRatioCommand (JSONArray request) {
	...
	public JSONArray execute() {
		...
		if (!errorCheck(response)) {
			try {
				int userRatioUpdated = projectDao.updateUserRatioInProject(projectName, targetUserName, week, ratio);
				response = addInt("userratioupdated", userRatioUpdated, response);
